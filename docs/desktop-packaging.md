...
CodeBundle uses `electron-updater` with GitHub Releases. Update checks run only in packaged builds, after the main window exists and a short startup delay. The app also exposes a manual **Check for Updates** action. Development builds never contact the release provider and show an unsupported message instead.

Downloaded updates are installed only after the user selects **Restart to Install**. Update checks send no project files, source code, telemetry, or credentials; they contact only the configured GitHub Release provider for application metadata/artifacts.
...
End-to-end update validation requires an installed older package and a newer GitHub Release whose package version and `v*` tag match, with the generated updater metadata attached. Credentialed release builds sign/notarize macOS artifacts and sign Windows artifacts. Unsigned beta builds can still show operating-system warnings.
...
- Release CI maps `WINDOWS_CSC_LINK` and `WINDOWS_CSC_KEY_PASSWORD` to electron-builder's `CSC_LINK` and `CSC_KEY_PASSWORD` only when both secrets are set. The configured publisher name must match the signing certificate subject.
- Use `scripts/verify-windows-signature.ps1` to inspect installer signatures. Linux artifacts remain unsigned in this phase.
...
- Credentialed release builds use Developer ID signing, hardened runtime, minimal Electron entitlements, electron-builder notarization, and ticket stapling.
- The PyInstaller sidecar is listed as an additional binary, so it is signed consistently with the app bundle before notarization.
...
Release CI supports signing only when complete release-owner credentials are present. Otherwise it explicitly disables certificate discovery and builds unsigned beta artifacts, preserving local contributor packaging. Set the repository variable `REQUIRE_CODE_SIGNING=true` to make missing macOS or Windows credentials fail a release job. Complete macOS releases require a Developer ID certificate plus Apple ID, app-specific password, and Team ID; Windows releases require a code-signing certificate. See [Code Signing and Notarization](code-signing-and-notarization.md) for setup, verification, and secret names.

## Current Limitation

Cross-platform sidecars still require platform-specific builds. Linux artifacts are unsigned in this phase, Windows SmartScreen reputation depends on certificate type and distribution history, and release checksums are future work. Release artifacts, installer outputs, PyInstaller outputs, sidecar binaries, certificates, and notarization credentials must not be committed.

Desktop production dependency audit (`npm audit --omit=dev`) is clean. Full `npm audit` has a separately tracked electron-builder development/build-tooling dependency chain issue that requires a breaking migration; do not use `npm audit fix --force` in this phase. See `desktop-security-upgrade.md` for package versions and earlier upgrade context.
...
The next packaging work should validate signed installed exports on each OS and consider release checksums or Linux distribution signing.
