...
End-to-end update validation requires an installed older package and a newer GitHub Release whose package version and `v*` tag match, with the generated updater metadata attached. Code signing and notarization remain separate work; unsigned beta builds can still show operating-system warnings.
